...
func TestAtomicPayment_DevModeBypass(t *testing.T) {
	...
	app := fiber.New()
	app.Post("/v1/scan/content", m.AtomicPayment(decimal.NewFromFloat(0.001)), func(c fiber.Ctx) error {
...
func TestRequirePayment_DevModeBypass(t *testing.T) {
	...
	app := fiber.New()
	app.Post("/v1/scan", m.RequirePayment(decimal.NewFromFloat(0.001)), func(c fiber.Ctx) error {
		return c.JSON(fiber.Map{"status": "ok"})
	})
...
func TestGetRoutes(t *testing.T) {
	...
	routeMap := make(map[string]decimal.Decimal)
	for _, r := range routes {
		routeMap[r.Path] = r.Price
	}

	assert.True(t, decimal.NewFromFloat(0.001).Equal(routeMap["/v1/scan/content"]))
	assert.True(t, decimal.NewFromFloat(0.001).Equal(routeMap["/v1/scan/output"]))
}
...
func TestDecimalToWei(t *testing.T) {
	testCases := []struct {
		...
		{decimal.NewFromFloat(0.001), "1000"},    // $0.001 = 1000 USDC atomic units (6 decimals)
		{decimal.NewFromFloat(0.01), "10000"},    // $0.01 = 10000 units
		{decimal.NewFromFloat(1.0), "1000000"},   // $1.00 = 1000000 units
		{decimal.NewFromFloat(0.000001), "1"},    // Smallest unit = $0.000001
	}
...
func TestDecimalToWei_Precision(t *testing.T) {
	...
	testCases := []struct {
		...
		{"very small amount", decimal.NewFromFloat(0.0000001), "0"},  // Below USDC precision
		{"one cent", decimal.NewFromFloat(0.01), "10000"},
		{"one dollar", decimal.NewFromFloat(1.0), "1000000"},
		{"large amount", decimal.NewFromFloat(1000000.0), "1000000000000"},
		{"fractional precision", decimal.NewFromFloat(0.123456), "123456"},
		{"nine cents", decimal.NewFromFloat(0.09), "90000"},          // Potential float representation issue
		{"nineteen cents", decimal.NewFromFloat(0.19), "190000"},     // Another common float issue
	}
